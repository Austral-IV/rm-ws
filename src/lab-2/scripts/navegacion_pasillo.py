...
class NavegacionPasillo(object):
  ...
  def find_wall_dist(self, depth_image):
    if depth_image is not None:
      ...
      lreading = sum(depth_image[wall_y, :wall_x])/wall_x

      rreading = sum(depth_image[wall_y, -wall_x:])/wall_x

      if lreading < rreading:
        self.lwall_dist = min(lreading * np.cos(np.deg2rad(self.fov/2)), self.hallway_width)
        self.rwall_dist = max(self.hallway_width - self.lwall_dist, 0)
      
      else:
        self.rwall_dist = min(rreading * np.cos(np.deg2rad(self.fov/2)), self.hallway_width)
